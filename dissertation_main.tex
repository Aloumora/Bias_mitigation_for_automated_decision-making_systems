...
\documentclass[twoside]{um}  % custom Polytechnic University of Turin project/dissertation/thesis 
...
\usepackage{nameref}
\usepackage{hyperref}
\usepackage[left=3cm, bottom=3.4cm,]{geometry}
...
\usepackage{listings}%per mettere il codice R
\lstset{%setta le opzioni per listings
  basicstyle=\ttfamily,
  columns=fullflexible,
  %frame=single, mette il frame attorno
  breaklines=true,
...
\definecolor{blue}{HTML}{33A4FF}
\definecolor{myGreen}{HTML}{009900}
%\setlength{\extrarowheight}{0.3cm}
\renewcommand{\familydefault}{\sfdefault}
\renewcommand{\arraystretch}{1.8}

%set colori x tabella capitolo 2
% sito per scegliere colori: https://www.htmlcsscolor.com/
\definecolor{SaffronDark}{HTML}{F6D87C}
\definecolor{SaffronLight}{HTML}{FAEBBC}
\definecolor{RoseDark}{HTML}{FDA1CD}
\definecolor{RoseLight}{HTML}{FDCFE5}
\definecolor{GreenDark}{HTML}{7EC3A1}
\definecolor{GreenLight}{HTML}{BDE1CF}

\colorlet{IEcolor}{SaffronDark}
\colorlet{IRcolor}{SaffronLight}
\colorlet{SUEcolor}{RoseDark}
\colorlet{SURcolor}{RoseLight}
\colorlet{SEEcolor}{GreenDark}
\colorlet{SERcolor}{GreenLight}

%% **************** (Your) Data (Start) ******************

\title{Bias mitigation for automated decision-making systems}  % use \\ here otherwise you get a justified title
                                                    % note capitalization of the title (only common 
                                                    % words in lower case)
\tagline{ }                     % tag line
\author{Alice Morano}                           % your name
\authorID{s259158}                           % your University Identifier
\supervisor{Dr. Antonio Vetrò}                             % your supervisor(s) name - no . in Dr
\cosupervisor{Elena Beretta}                               % your cosupervisor(s) name - no . in Dr ** OPTIONAL ** 
                                                    % simply comment out the above line if absent
\department{Department of Control and Computer Engineering}                  % your department (e.g. Artifical Intelligence)
\faculty{Computer Science}                      % your faculty (e.g. ICT)
\degree{M.Sc.\ in Computer Engineering}                      % the degree you are reading
                                                    % note the \ after the dot, so not to consider it a fullstop
\doctype{thesis}                              % the type of document (fyp, dissertation, thesis)
\degreedate{December 2020}                        % when did you submit -- officially after your corrections !
...
\frontmatter 
    ...
    \input{frontmatter/summary}\if@openright\cleardoublepage\else\clearpage\fi   % include an acknowledgements.tex file
...
{fontsize=\footnotesize,
 ...
 %
 label=\fbox{\color{#000000}COMPAS fairness metrics data},
...
\mainmatter 
    ...
    \input{chap3/research_goals_main}
    ...
    \appendix
        ...
        \input{appB/appendix_b_main}\label{appB}    
...
{\backmatter
    ...
    %um-plainnat
    \bibliographystyle{plain} %% specific plainnat does not show url for articles
